# adaptive_exit.py — V8.5.2 President Intelligence Merge
# -------------------------------------------------------
# V7 Adaptive Exit fikrinin V8 President mimarisine güvenli entegrasyonu.
# Bu modül EMİR AÇMAZ, POZİSYON KAPATMAZ. Sadece President OPEN kararından
# sonra kullanılacak exit policy / trade class üretir. Nihai karar President'tadır.
...
DEFAULT_ADAPTIVE_EXIT_CFG: Dict[str, Any] = {
    ...
    "policies": {
        "TREND_RUNNER": {
            "tp1_close_pct": 0.08, "trail_step_pct": 2.20, "max_hold_hours": 72,
            "breakeven_after_r": 1.0, "breakeven_buffer_pct": 0.25,
            "allow_trend_runner": True, "size_mult": 1.0,
        },
        "NORMAL": {
            "tp1_close_pct": 0.35, "trail_step_pct": 1.00, "max_hold_hours": 48,
            "breakeven_after_r": 1.0, "breakeven_buffer_pct": 0.15,
            "allow_trend_runner": False, "size_mult": 1.0,
        },
        "SCALP_EXIT": {
            "tp1_close_pct": 0.45, "trail_step_pct": 0.75, "max_hold_hours": 18,
            "breakeven_after_r": 0.80, "breakeven_buffer_pct": 0.12,
            "allow_trend_runner": False, "size_mult": 0.85,
        },
        "CHOP_RISK": {
            "tp1_close_pct": 0.50, "trail_step_pct": 0.60, "max_hold_hours": 12,
            "breakeven_after_r": 0.70, "breakeven_buffer_pct": 0.10,
            "allow_trend_runner": False, "size_mult": 0.75,
        },
        "EXHAUSTED": {
            "tp1_close_pct": 0.60, "trail_step_pct": 0.60, "max_hold_hours": 12,
            "breakeven_after_r": 0.70, "breakeven_buffer_pct": 0.10,
            "allow_trend_runner": False, "size_mult": 0.75,
        },
        "DISABLED": {
...
def get_adaptive_exit_config(cfg: Optional[Mapping[str, Any]]) -> Dict[str, Any]:
    return _deep_merge(DEFAULT_ADAPTIVE_EXIT_CFG, (cfg or {}).get("adaptive_exit", {}) or {})
...
def _symbol_score(symbol_stats: Optional[Mapping[str, Any]], cfg: Mapping[str, Any]) -> Tuple[float, float, str]:
    ...
    min_closed = int(sb_cfg.get("min_closed_trades", 20))
...
def _policy_from_cfg(name: str, cfg: Mapping[str, Any]) -> ExitPolicy:
    ...
    return ExitPolicy(
        name=name,
        tp1_close_pct=_float(p.get("tp1_close_pct"), 0.35),
        trail_step_pct=_float(p.get("trail_step_pct"), 1.0),
...
def classify_trade(
    *, symbol: str, side: str = "LONG", score: float = 0.0, htf_score: float = 50.0,
    regime: str = "UNKNOWN", components: Optional[Mapping[str, Any]] = None,
    cfg: Optional[Mapping[str, Any]] = None, prices: Optional[Iterable[float]] = None,
    highs: Optional[Iterable[float]] = None, lows: Optional[Iterable[float]] = None,
    volumes: Optional[Iterable[float]] = None, current_r: float = 0.0,
    btc_trend_ok: Optional[bool] = None, symbol_stats: Optional[Mapping[str, Any]] = None,
) -> AdaptiveExitDecision:
    ae_cfg = get_adaptive_exit_config(cfg or {})
    enabled = bool(ae_cfg.get("enabled", True))
    shadow = bool(ae_cfg.get("shadow_mode", False))
    ...
    reasons = []
    cont = 45.0
    if score >= 98: cont += 8; reasons.append("score>=98 +8")
    elif score >= 95: cont += 5; reasons.append("score>=95 +5")
    elif score >= 90: cont += 2; reasons.append("score>=90 +2")
    else: cont -= 8; reasons.append("score<90 -8")

    if htf_score >= 85: cont += 10; reasons.append("htf>=85 +10")
    elif htf_score >= 70: cont += 7; reasons.append("htf>=70 +7")
    elif htf_score >= 55: cont += 2; reasons.append("htf>=55 +2")
    else: cont -= 12; reasons.append("htf<55 -12")

    if adx >= 35: cont += 12; reasons.append("adx>=35 +12")
    elif adx >= 28: cont += 8; reasons.append("adx>=28 +8")
    elif adx >= 22: cont += 4; reasons.append("adx>=22 +4")
    elif adx > 0: cont -= 8; reasons.append("adx_weak -8")

    if 50 <= rsi <= 66: cont += 9; reasons.append("rsi_healthy +9")
    elif 66 < rsi <= 70: cont += 2; reasons.append("rsi_warm +2")
    elif rsi > 72: cont -= 22; reasons.append("rsi_exhausted -22")
    elif rsi < 45: cont -= 8; reasons.append("rsi_low -8")

    if vol_ratio >= 1.6: cont += 8; reasons.append("vol>=1.6 +8")
    elif vol_ratio >= 1.15: cont += 4; reasons.append("vol>=1.15 +4")
    elif vol_ratio < 0.85: cont -= 6; reasons.append("vol_dry -6")

    if 1.0 <= atrp <= 3.5: cont += 5; reasons.append("atr_ok +5")
    elif atrp > 5.0: cont -= 10; reasons.append("atr_too_high -10")
    elif 0 < atrp < 0.7: cont -= 8; reasons.append("atr_low -8")

    if btc_trend_ok is True: cont += 5; reasons.append("btc_ok +5")
    elif btc_trend_ok is False: cont -= 18; reasons.append("btc_bad -18")
    else: reasons.append("btc_unknown +0")

    is_chop = regime_u in CHOP_REGIMES
    if regime_u in TREND_REGIMES: cont += 8; reasons.append("regime_trend +8")
    elif is_chop: cont -= 30; reasons.append("regime_chop_guard -30")
    elif regime_u in RISK_REGIMES: cont -= 18; reasons.append("regime_risk -18")

    if current_r >= 2.0: cont += 4; reasons.append("current_r>=2 +4")
    elif current_r >= 1.0: cont += 2; reasons.append("current_r>=1 +2")
    ...
    if abs(sym_impact) > 0.01:
        cont += sym_impact; reasons.append(f"symbol_behavior {sym_impact:+.1f}")
    else:
    ...
    trend_gate_ok = True
    gate_failures = []
    if is_chop: trend_gate_ok = False; gate_failures.append("chop")
    if score < _float(gates.get("min_entry_score"), 95): trend_gate_ok = False; gate_failures.append("score_gate")
    if htf_score < _float(gates.get("min_htf_score"), 70): trend_gate_ok = False; gate_failures.append("htf_gate")
    if adx < _float(gates.get("min_adx"), 22): trend_gate_ok = False; gate_failures.append("adx_gate")
    if rsi > _float(gates.get("max_rsi"), 70): trend_gate_ok = False; gate_failures.append("rsi_gate")
    if vol_ratio < _float(gates.get("min_vol_ratio"), 0.85): trend_gate_ok = False; gate_failures.append("vol_gate")
    if bool(gates.get("require_btc_ok", False)) and btc_trend_ok is not True:
    ...
    if str(side).upper() == "SHORT" and trade_class == "TREND_RUNNER":
        trade_class = "NORMAL"; reasons.append("short_runner_disabled")

    policy = _policy_from_cfg(trade_class, ae_cfg)
    confidence = max(0.0, min(100.0, abs(cont - 50.0) * 2.0))
    return AdaptiveExitDecision(True, shadow, trade_class, round(cont, 3), round(confidence, 3), policy, "; ".join(reasons), regime_u, round(sym_score, 3), round(sym_weight, 3))


def decision_note(decision: AdaptiveExitDecision) -> str:
    return f"AE class={decision.trade_class} cont={decision.continuation_score:.1f} conf={decision.confidence:.1f} policy={decision.policy.name} shadow={int(decision.shadow_mode)} regime={decision.regime}"
