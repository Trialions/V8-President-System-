    ...
    try:
        ...
        if os.path.exists(tp):
            with open(tp, newline="", encoding="utf-8-sig") as f:
                result["trades"] = list(csv_mod.DictReader(f, delimiter=";"))[:500]
        # Equity (max 300 nokta)
        ...
        if os.path.exists(fp):
            with open(fp, newline="", encoding="utf-8-sig") as f:
                result["filter_events"] = list(csv_mod.DictReader(f, delimiter=";"))[:200]
        # Ghost
        gp = os.path.join(path, "ghost_signal_analysis.csv")
        if os.path.exists(gp):
            with open(gp, newline="", encoding="utf-8-sig") as f:
                result["ghost"] = list(csv_mod.DictReader(f, delimiter=";"))[:200]
